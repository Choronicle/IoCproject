...
public class CartController {
    ...
    @GetMapping("/")
    public List<Integer> load(HttpServletRequest request, HttpServletResponse response){
    ...
    public boolean deleteCart(@Param("item_id") Integer item_id,  HttpServletRequest request, HttpServletResponse response){
        HttpSession session = request.getSession();
        User user = (User) session.getAttribute("user");
        if(user == null) {
            response.sendRedirect();//TODO:login页面跳转路径
